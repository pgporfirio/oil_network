...
## Resume here (2026-05-26 — v2.7: Chapter 5 fully restructured as four change-management scenarios)

**TL;DR.** Latest on-disk:

- **`outputs/docs/Master_Thesis_Pedro_Porfirio_restructured_v2.7.docx`** (2026-05-26, 1.18 MB — embeds Figure 5.1 + 6 comparison tables)
- **`outputs/docs/Master_Thesis_Pedro_Porfirio_restructured_v2.7.pdf`** (2026-05-26, 1.27 MB) — paired PDF, 55 pages, ~18.6k words.
- **`outputs/docs/fig_5_1_jones_act_corridor.svg`** (source) + **`outputs/docs/figures/fig_5_1_jones_act_corridor.png`** (render embedded in docx — same as v2.6, caption reframed).

v2.6, v2.5, v2.4, and earlier restructure passes remain on disk for diff/reference. **Do not edit or quote from the old v37–v46 series** — superseded.

**The reframe in v2.7.** v2.6 framed Chapter 5 as steady-state numerical comparison. Pedro pushed back: the framework's "better" is not about producing different balance numbers today but about **how it absorbs change**. v2.7 restructures the whole chapter around four change-management scenarios. Each is a 3-method comparison (Excel spreadsheet | Pandas notebook | Framework) across five dimensions (what changes, how to assess, effort hours, consistency risk, failure mode):

| § | Scenario | Type of change | Hours: Excel / Pandas / Framework |
|---|---|---|---|
| 5.2 | **Adding grade dimensions to basin production** | data-dimension widening | 60–120 / 30–60 / **4–8** |
| 5.3 | **Refinery TAR / unplanned outage** (Harvey as illustration) | resolution refinement | 40–80 + 4–8/event / 20–40 + 2–4/event / **2–4/event** |
| 5.4 | **Receiving Genscape pipeline data** | new data source | 20–40 + 2–4/pipeline / 15–30 + 1–3/pipeline / **N × 1–2** |
| 5.5 | **Finding inconsistencies** | consistency-audit infrastructure | 5–20 + 1–2/mo / 10–30 + 2–5/scenario / **0 (built-in)** |

Each section follows the same shape: setup paragraph → bold-led Excel / Pandas / Framework narratives → comparison table. §5.6 aggregates with a cost-asymmetry table (Table 5.6) and a synthesis paragraph: *"The framework is not a methodologically more sophisticated balance — it produces the same monthly aggregate values from the same EIA series in steady state. It is a representation that absorbs change without requiring rebuild, and that enforces consistency at the schema level rather than relying on analyst vigilance."*

**Tables in v2.7:**

- Table 5.1 — Adding grades comparison (replaces v2.6's PADD 5 stock identity)
- Table 5.2 — TAR/outage comparison (replaces v2.6's PADD 3 Harvey monthly balance; Harvey now in prose as illustration)
- Table 5.3 — Genscape comparison (replaces v2.6's Genscape layering)
- Table 5.4 — Finding inconsistencies comparison (replaces v2.6's named-events table at this position)
- **Table 5.5** (new) — Named events |B| at USA aggregate (Harvey, COVID, Colonial, SPR), moved here as evidence for §5.5
- **Table 5.6** (new) — Aggregate cost asymmetry across all four scenarios

**Figure 5.1 unchanged**, caption updated to reframe as the grade-widening illustration (top panel = single-grade structural placement context; bottom panel = per-grade widening).

**Dropped from v2.6:**

- §5.2 Jones Act in-transit anomaly as a primary case study — the Jones Act corridor survives only as the illustrative asset in Figure 5.1 and a brief textual mention.
- §5.3 Hurricane Harvey as a primary case study — Harvey is now a worked example *within* §5.3 (TAR/outage), in prose with the +193 kbd PADD 3 residual mentioned but without its own table.
- v2.6 Table 5.2 (PADD 3 monthly balance, August 2017) is gone.
- The "Steady-state comparison" / "Stress-test comparison" / "Multi-source integration" framings are gone from the section titles — all four are now change-management scenarios.

**Implementation.** [code/rewrite_chapter5_v27.py](../code/rewrite_chapter5_v27.py) uses python-docx 1.2.0 to:

- Walk Chapter 5 body elements, identify section headings, delete all bodies (keeping headings and Figure 5.1).
- Rewrite chapter intro paragraph + each section heading text in place.
- Insert new content (prose paragraphs with bold lead-ins for the method narratives; tables built fresh with custom borders since the template lacks `Table Grid` style).
- Move Figure 5.1 to the end of the new §5.2 body and update its caption.
- Update TOC and LoT entries throughout.
- Clone the last LoT row to add entries for new Tables 5.5 and 5.6.

**Known follow-ups (not blockers):**

- **Page numbers in TOC, LoT, LoF are still static text** and have not been refreshed to v2.7's new layout. The body content and cross-references are correct; only the page-number metadata is potentially stale. Pedro should refresh by Ctrl-A + F9 in Word, or eyeball each entry against the PDF.
- **New tables use a custom border style** (single-line 0.5pt grey #666666 on all sides + insideH/insideV) since the v2.4-derived template doesn't carry a `Table Grid` style. If the visual treatment differs from how Pedro wants it, the borders can be tuned in `_add_table_borders()` in [rewrite_chapter5_v27.py](../code/rewrite_chapter5_v27.py).
- **Hour numbers are rough order-of-magnitude estimates** documented as such in §5.1 and §5.6. If Pedro has experience-based numbers to substitute, drop them in.

**DB state unchanged** — 251 / 1,870 / 975 / 137 / 68,793 / 291,564 / 0 unresolved (verified 2026-05-26).

---

## Earlier (2026-05-26 — v2.6: Chapter 5 §5.2 rewritten with orthogonality framing + new Figure 5.1)

v2.6 framed §5.2 around the Jones Act in-transit anomaly with the labelling-tie + structural-placement difference + per-grade widening argument. Superseded by v2.7's four-scenario restructure.

**TL;DR.**

- **`outputs/docs/Master_Thesis_Pedro_Porfirio_restructured_v2.6.docx`** (2026-05-26, 1.18 MB — embeds Figure 5.1)
- **`outputs/docs/Master_Thesis_Pedro_Porfirio_restructured_v2.6.pdf`** (2026-05-26, 1.28 MB) — paired PDF, 56 pages, ~18.8k words.
- **`outputs/docs/fig_5_1_jones_act_corridor.svg`** (source) + **`outputs/docs/figures/fig_5_1_jones_act_corridor.png`** (render embedded in docx).

v2.6 + earlier remain on disk for diff/reference. **Do not edit or quote from the old v37–v46 series** — superseded.

**What changed in v2.6 (Chapter 5 rewrite):**

1. **§5.2 retitled** "the PADD 5 in-transit anomaly" → "**the Jones Act in-transit anomaly**" (updated in TOC, body heading, §5.1 forward-reference, and §5.6 backward-reference — 4 occurrences total).
2. **§5.2 prose fully rewritten** — was 6 paragraphs (~700 words) of strawman-spreadsheet framing where the workbook "lumps 6 mb into an unallocated adjustment". v2.6 is 11 paragraphs (~1,300 words) with a different argument:
   - **Stage 1 (concede the labelling tie)**: a competent spreadsheet analyst binds to EIA's published in-transit-by-water series and labels the row correctly; PADD 5 partition closes in both methods. *"The labelling itself is not what distinguishes the two methods."*
   - **Stage 2 (the structural-placement difference)**: spreadsheet's labelled row is a *flat cell* with no neighbours, capacity ceiling, flow variable, or mass-balance link. Framework's `inter_padd_3_to_5_agg` is a directed physical asset with all of those, including per-direction flow variables on the edges to PADD 3 and PADD 5.
   - **Stage 3 (the data-layer widening test — the orthogonality payoff)**: when the desk asks "what grades are in transit, and how do they propagate through the system?", the spreadsheet must rebuild (every column duplicates by grade, every reconciliation rule redefined); the framework's topology stays bit-identical and only the variables table widens 1,870 → 1,870 × N. The argument is *structural, not empirical* (per-grade not implemented in stage 1; explicitly flagged).
3. **Table 5.1 cells updated**:
   - "Commercial tank farms" spreadsheet col: `~22,962` → `22,962` (dropped tilde to reflect tied labelling)
   - "Refinery stocks" spreadsheet col: `~20,468` → `20,468`
   - Row label "In-transit from PADD 3" → "In-transit by water from PADD 3"
   - In-transit spreadsheet cell: `(absent: 5,900 lumped into the residual below)` → `5,900 (row labelled 'in-transit')`
   - In-transit framework cell: `5,900 (TS-bound on inter_padd_3_to_5_agg)` → `5,900 (TS-bound to inter_padd_3_to_5_agg.inventory)`
   - "Unallocated adjustment" row: `+5,900 / 0` → `0 / 0`
   - Caption rewritten to flag Figure 5.1 cross-reference.
4. **New Figure 5.1** (Concept B — side-by-side comparison on top, per-grade extension panel below) — SVG source committed at `outputs/docs/fig_5_1_jones_act_corridor.svg`, PNG render at `outputs/docs/figures/fig_5_1_jones_act_corridor.png` (552 KB), embedded centred at width 6 inches in the docx. List of Figures entry added (cloned from Figure 4.1 entry).
5. **Table renumbering — fixes the v2.5 duplicate "Table 5.4" bug**:
   - §5.4 (Genscape) caption + in-text reference: `Table 5.4` → `Table 5.3`
   - §5.5 (named events) caption + in-text reference: `Table 5.4` (stays, since it's the fourth chronological table)
   - (The List of Tables front-matter was already correct in v2.5 — it numbered them 5.1 / 5.2 / 5.3 / 5.4 even though the body had two "Table 5.4"s; v2.6 brings the body in line with the LoT.)
6. **Hurricane Harvey magnitude clarifier inserted in §5.5** — addresses the previously-flagged inconsistency that Harvey at the USA aggregate is 0.6× median (below) while surrounding prose says "rises notably above". New sentence: *"Hurricane Harvey at the USA aggregate is the partial exception: its national-level signal is partly cancelled by offsetting flows between PADD 3 and the rest of the country, while the per-PADD signal documented in §5.3 remains clear."*

**Implementation notes.** The rewrite was applied by [code/rewrite_chapter5_v26.py](../code/rewrite_chapter5_v26.py) using python-docx 1.2.0 (which is in the venv). The script:
- Updates §5.2 heading text + TOC entry + §5.1 forward-ref + §5.6 back-ref via direct XML `<w:t>` replacement.
- Modifies Table 5.1 cells in-place (preserves existing table styling).
- Deletes old §5.2 prose paragraphs (4 before table + 3 after caption) by `body.remove()`.
- Inserts new paragraphs using `prev.addnext(make_para(text))` — preserves paragraph order without disturbing the surrounding sectPr.
- Inserts Figure 5.1 by `doc.add_paragraph().add_run().add_picture(png, width=Inches(6.0))` then `addnext()` to position.
- Renumbers §5.4 references by `find_and_replace_text(el, 'Table 5.4', 'Table 5.3')` scoped to the first two occurrences (which are the §5.4 ones; the last two are §5.5).
- Inserts Harvey clarifier by anchored-phrase replacement.

**Known follow-ups (not blockers — flag on next thesis pass):**

- **Page numbers in TOC, List of Tables, List of Figures are static text, not Word TOC fields** — so they did not auto-update when §5.2 grew by ~600 words. The LoF entry for Figure 5.1 carries page `24` as a placeholder. Pedro should refresh page numbers manually in Word (Ctrl-A + F9 if any are field-based; otherwise scan each TOC/LoF/LoT entry against the actual PDF). The body content and references are all correct; only page-number metadata is potentially stale.
- **No `433 directed flow edges` claim drift** — that number was verified against `v_flow_edges` row count on the live DB (2026-05-26). Safe to quote.

**DB state unchanged from 2026-05-26 verification** — 251 / 1,870 / 975 / 137 / 68,793 / 291,564 / 0 unresolved. See verification block lower in this handover.

---

## Earlier (2026-05-26 — v2.5: principle-numbering "Option A" fix)

**TL;DR.** v2.5 differs from v2.4 only by the 8-edit principle-numbering fix in §§4.1–4.6 + §5.4 cross-reference (see "Five-vs-six principles — RESOLVED in v2.5" below). v2.4 + its PDF remain on disk for diff. Earlier restructure passes (`restructured_v2.0` 2026-05-22 → `v2.1` → `v2.2` → `v2.3` 2026-05-22..25) are all on disk for history.

**Size collapse.** v46 was **44,141 words**; restructured v2.4 is **17,752 words** (–60%). v2.0 was 16,492 words, so v2.0→v2.4 added ~1.3k words of refinement (mostly the case study). Bulk previously in main-body implementation chapters is now relocated to Annex A or cut entirely.

**New structure — 6 chapters + 3 annexes.** (Page numbers from the v2.4 TOC.)

| # | Title | Pages |
|---|---|---|
| 1 | Introduction (background, problem, objectives, scope, structure) | 1 |
| 2 | Current techniques for producing oil balances (official balances, practitioner spreadsheets, optimisation, graph/ML, **§2.5 the gap**) | 8 |
| 3 | Domain background (U.S. crude oil network, multi-resolution reporting structure, requirements) | — |
| 4 | A graph-and-scenario framework (§§4.1–4.6 principles, §4.7 synthesis) | 16 |
| 5 | Case study: where the framework is better (§§5.1 setup, **5.2 PADD 5 in-transit anomaly**, **5.3 Hurricane Harvey Aug 2017**, **5.4 multi-source pipeline-flow integration**, 5.5 other named events, 5.6 establishes) | 22 |
| 6 | Conclusion and future work | 31 |
| Refs | References | 34 |
| A | **Implementation** (schema, starter U.S. graph, the resolver, consistency properties demonstrated on the live system) | 37 |
| B | **Variable catalogue and starter scenario inventory** (types, authoritative bindings by subsystem, latents by location) — new annex | — |
| C | **Graph neural networks: a primer** (node embeddings/message passing, GCN, temporal extensions, GNN-readiness) — was previously Annex A | — |

**Framework chapter is reframed as principles, not the old §2 list of 10.** Chapter 4 prose declares "five design principles" but the document has six numbered subsections §§4.1–4.6 plus a §4.7 synthesis ("What the principles deliver together"). The mapping from the old `claude/CLAUDE.md` Section 2 (committed 10 principles):

- 4.1 **Physical infrastructure is orthogonal to data** — new framing; folds in 2.5 (persistent asset graph vs scenario state).
- 4.2 **Assets are first-class nodes** — was 2.1 (asset-centric representation), absorbs 2.2 (zero-flow edges) and 2.10 (bidirectional flows as separate edges) as implementation consequences.
- 4.3 **Mass balance with an explicit balancing item** — was 2.3.
- 4.4 **Observed or derived: single attribution at the schema level** — was 2.8 (the most important invariant), absorbs 2.4 (formula-implies-relation).
- 4.5 **Latent variables for unobserved flows** — was 2.9.
- 4.6 **Multi-resolution data through aggregation views** — folds 2.6 (physical vs observational layer) and 2.7 (geography metadata ≠ resolution hierarchy) into one.
- 4.7 Synthesis — "what the principles deliver together".

*Five-vs-six principles — RESOLVED in v2.5 (2026-05-26).* v2.4 had three internal inconsistencies on the count: the intro/§4.7/§6.1 said "five"; §4.1 closing said "the four principles that follow"; the callouts in §§4.1–4.6 labelled them "Principle 1" through "Principle 6". **Option A** chosen: orthogonality (§4.1) becomes the unnumbered framing principle; §§4.2–4.6 become Principles 1–5 (operational consequences). Eight surgical edits applied to v2.5.docx via direct `<w:t>` replacements on `word/document.xml`:

1. §4.1 callout: `Principle 1 — Orthogonality.` → `Orthogonality.`
2. §4.1 closing: `the four principles that follow` → `the five principles that follow`
3. §4.2 callout: `Principle 2 — Assets as nodes.` → `Principle 1 — Assets as nodes.`
4. §4.3 callout: `Principle 3 — Mass balance with B.` → `Principle 2 — Mass balance with B.`
5. §4.4 callout: `Principle 4 — Observed or derived.` → `Principle 3 — Observed or derived.`
6. §4.5 callout: `Principle 5 — Latent flows.` → `Principle 4 — Latent flows.`
7. §4.6 callout: `Principle 6 — Abstract nodes do not have edges.` → `Principle 5 — Abstract nodes do not have edges.`
8. §5.4 cross-reference: `the data layer — Principle 1 — is what makes this true` → `the data layer is what makes this true`

Untouched (still correct): chapter intro "**five** design principles", §1.5 thesis-structure preview, §4.7 "**five** principles produce a representation with **four** properties", §6.1 "rests on **five** principles". Post-edit grep confirms exactly one occurrence each of `Principle 1`..`Principle 5` and zero of `Principle 6`. PDF regenerated via `docx2pdf` (Word backend); ~1.13 MB, parity with v2.4.pdf.

**Phi cleanup landed.** The 2026-05-21 handover entry below flagged that the `phi` variable type still appeared in §5.6 and Annex D.2 of v45/v46. v2.4 has **0 occurrences** of "phi" (word, case-insensitive) and **0 occurrences** of the Greek letters ϕ/φ. The §5.6 phi paragraph and Annex D.2 phi concession are gone, and the LP-notation `ϕ(i,j,g,t)` from old Chapters 8–9 is also absent (the old LP chapters are gone too — they were part of the bulk Pedro cut from main body). No remaining phi cleanup task on the thesis.

**Variable-types canonical list now consistent with the schema.** "balancing item" appears 30 times in v2.4 body; the canonical six (production, consumption, inflow, outflow, inventory, balancing_item) match `oil_network.variable_types` exactly. No more "seven" framings.

**DB state — verified 2026-05-26, matches `stage_1_complete` snapshot exactly:**

| Metric | Value | Expected |
|---|---|---|
| assets / nodes | 251 / 251 | 251 (the literal in `verify_state.py` says 252 — off-by-one bug in the script, not data) |
| variables | 1,870 | 1,870 |
| variable_assignments | 975 | 975 |
| timeseries (catalogue) | 137 | 137 |
| timeseries_data | 68,793 | 68,793 |
| scenario_resolved_values | 291,564 | 291,564 |
| unresolved rows | 0 | 0 |
| TS-binding collisions | 0 | 0 |
| capacity violations @ 2024-12-01 | 0 | 0 |

Resolver dispatch (119,652 latent + 84,552 zero + 57,096 partial + 16,224 derived + 14,040 observed). Partition closure at USA/PADD level: 0 red cells; PADD 4/5 `consumption` rows show `partial_coverage` as documented (EIA does not publish PADD-level consumption finer than aggregate). Spot-checks show 4 out-of-range as previously documented (USA consumption slightly high; USA outflow/inflow MISSING; Permian-TX prod low — all pre-existing, not new drift).

**Documentation drift to be aware of.**

- [CLAUDE.md](../CLAUDE.md) — the "Commands" section tells you to use `..\..\.venv\Scripts\python.exe`, but the venv actually lives one level up at `..\.venv` (i.e. `Oil Network Project/.venv`). Stale because Stage1 sits one level under the project root, not two. Worth correcting in CLAUDE.md.
- [claude/CLAUDE.md](CLAUDE.md) — Sections 2 (10 design principles), 4 (what has been built), 6 (next workstream), and 9 (file layout) describe pre-restructure state. The principles in §2 are still authoritative for the *code* but no longer map 1:1 to the thesis chapter — use the 4.x mapping above to bridge. Section 9 lists scripts (`twelfth_pass_cleanup.py`, `thirteenth_pass_views.py`, `make_balance_ui.py`, `pdf_graph_construction.py`, etc.) that don't match the current `code/` directory; the real list is governed by [claude/NOTEBOOKS.md](NOTEBOOKS.md).
- The 2026-05-21 entry below references `outputs/docs/Master_Thesis_Pedro_Porfirio_v46.docx` as the latest and the v47 phi cleanup as planned. **Both are now superseded by v2.4.** Read that entry as historical.

**Open questions on resume.**

1. Should the old numbered v37–v46 `.docx` files in `outputs/docs/` be moved to `outputs/docs/old/` or `Thesis/old/` to keep the active surface clean? They are 1.1–1.5 MB each and noise in directory listings. Same question for restructured v2.0–v2.4 once v2.5 is comfortable as canonical.
2. CLAUDE.md update — both the venv path (`..\.venv` not `..\..\.venv`) and the post-restructure section mappings — is worth one pass when convenient.

**Thesis-edit backlog (deferred, not yet applied to v2.4).**

- **Gulf-Coast naming.** §3.1 currently uses *"Gulf of America (formerly Gulf of Mexico)"*. Pedro's preference is **USGC (US Gulf Coast)** throughout the thesis. Hold the change until a future editing pass — not urgent, but apply consistently when next opening the docx (likely affects §3.1, possibly §§3.3, 5.x, A.2). Decision logged 2026-05-26.

---

## Earlier (2026-05-25 — silent-failure mode found in rebuild orchestrator; patched + canonical state restored)

**Context.** Pedro tried a full validation rebuild via `code/initialize_oil_network.ipynb` in VS Code. All four stages reported ✓ in 76 seconds total. The DB looked broken: 232 nodes / 1,756 vars / **0 timeseries_data rows** / **0 scenario_resolved_values** / all spot-checks missing.

**Root cause — VS Code cwd vs notebook directory.**

The master notebook had:
```python
load_dotenv(Path.cwd().parent / ".env")
```

When VS Code runs a notebook, `cwd` is the **workspace root**, not the notebook's directory. With `Stage1/` opened as the workspace, `Path.cwd().parent` resolved to `Oil Network Project/`. `.env` lives at `Stage1/.env`, not `Oil Network Project/.env` — so `load_dotenv` silently failed, `EIA_API_KEY` never landed in `os.environ`, child subprocesses ran without it.

Stage 3's EIA loader is designed to **silently no-op** when no API key is present (returns exit code 0 with empty `oil_network_data_loader` schema). Stage 4 then ran assignments and the resolver, but with no TS data the resolver wrote zero rows to `scenario_resolved_values` — and also returned exit code 0 because nothing actually crashed. **The pipeline reported four green stages while doing essentially nothing data-wise.**

This is the worst class of failure: green CI, empty DB.

**Fix applied — `code/initialize_oil_network.ipynb` cell 4 patched.** Two changes:

1. **Robust .env loading.** Walk UP from cwd looking for `.env` in each parent, stop at the first one found. Prints the loaded path or warns clearly if none found. Works regardless of whether the notebook is launched from `code/`, `Stage1/`, or anywhere else.
2. **Hard EIA_API_KEY assertion BEFORE running any stages.** Raises `RuntimeError` immediately if the key is missing, with a message explaining the silent-failure mode this prevents. No more "green pipeline, empty DB".

The patched cell now:
```python
try:
...
if not os.environ.get("EIA_API_KEY"):
    raise RuntimeError("EIA_API_KEY is not set. Add it to Stage1/.env ...")
```

**Other silent-failure surfaces worth knowing about** (NOT yet fixed; flag on resume):

- **Stage 3 EIA loader returns 0 with empty schema** when API key is missing. The loader itself should ideally raise; until it does, the orchestrator's pre-check above is the only guard. Same pattern likely exists for any future commercial data source (CER, AER, Genscape) — they should ALL raise on missing creds rather than silently no-op.
- **Stage 4 resolver writes to a row in `scenario_resolver_runs` at the start** (run_id increments, `started_at` populated, but `completed_at` / `n_rows_written` left NULL until the run finishes). A NULL `completed_at` is the post-hoc tell that the resolver crashed mid-run — `verify_state.py` could check this as part of its sanity sweep but currently doesn't.
- **`verify_state.py` itself reports raw numbers but doesn't ABORT or non-zero-exit on bad state** — its output has to be eyeballed. Could be extended to fail loudly when key counts are zero.

**Canonical state restored from snapshot, all HTMLs regenerated.**

After diagnosis, restored from `..\snapshots\oil_network_stage_1_complete.dump` (drop oil_network CASCADE first, then `pg_restore -n oil_network` into the empty schema). All canonical numbers match: **251 / 1,870 / 975 / 137 / 68,793 / 291,564 / 0 unresolved**. All 5 HTMLs regenerated against the restored DB (run_id=13, generated 2026-05-25 ~14:30Z).

**Test the patch on resume.** The patch was applied but not re-run end-to-end (would have destroyed the canonical state we just restored). To validate: on a future rebuild, deliberately delete or move `.env`, run the notebook, and confirm it raises with the new clear error message instead of producing an empty DB. Then restore `.env` and run normally to confirm it loads from the find-up walk.

---

## Earlier (2026-05-21 — stage-1 thesis-defence focus; phi found unused and slated for removal from v47)

**Context.** Stage 2 (per-grade resolver propagation) is **paused indefinitely** — focus is now on stage 1 as the thesis defence artefact. v47 of the thesis is in progress in Word (not yet saved to disk; latest on-disk version is `outputs/docs/Master_Thesis_Pedro_Porfirio_v46.docx`, with v45 the latest with paired PDF). The earlier resume entry below (2026-05-18) about resuming stage 2 is **superseded by this one** — do not pick up the stage 2 resolver redesign without explicit re-direction.

**DB / framework state unchanged from stage 1 close.** 251 assets, 1,870 variables, 975 assignments, 291,564 resolved rows, 19 commodities, 18 hierarchy edges, 0 unresolved. `stage_1_complete` tag still pinned. Snapshot at `Oil Network Project/snapshots/oil_network_stage_1_complete.dump`.

**Open thesis edit decided this session: remove the `phi` variable type from the thesis prose.**

*Findings — phi is not in the stage-1 schema or resolver:*

- `oil_network.variable_types` contains exactly **six** rows: `production, consumption, inflow, outflow, inventory, balancing_item`. There is no `phi` value in the enum.
- `variables` has **0 rows** with `variable_type='phi'`; `variable_assignments`, `scenario_resolved_values`: 0 phi rows.
- Neither `resolve_scenario.py` nor `recursive_resolver.py` references "phi" anywhere. `network_graph.py` has zero meaningful matches (one false positive on "Geographic" via case-insensitive grep).
- Annex D.2 already concedes the count is zero ("phi count of zero reflects the rollback of an interim grade-decomposition trial… reintroduced when grade-level work becomes the next workstream"). Since stage 2 is paused, that future-work hedge no longer needs to be in the thesis.

*Two distinct uses of "phi" in v45 — only one is the deletion target:*

1. **The "phi variable type" prose — DELETE.** §5.6 Variables (para 273 in v45) opens with *"A note on the phi variable. The phi type is a cross-node alias reference…"* and describes phi as a seventh structural-pointer variable type alongside the other six. Annex D.2 has the matching concession sentence. Both should go. Reframe the variable-type list as **six** types throughout the thesis (production, consumption, inflow, outflow, inventory, balancing_item) to match the schema, resolver, and the dispatch counts in `claude/PROJECT_STATE.md` (90 observed + 542 zero + 767 latent + 451 arithmetic + 5 sum + 15 reverse_mirror).
2. **The `ϕ(i, j, g, t)` LP notation in Chapters 8–9 — KEEP, or rename to `f`.** This is the LP decision variable for per-edge flow in §8.2 (Mass balance as LP rows), §8.4 (Permian fan-out worked example), §8.5 (Multi-period LP), and §9.2.1–9.2.2 (case study LP). It is *not* the same object as the §5.6 phi variable type — it is standard network-flow LP notation. Optional cleanup: find-and-replace `ϕ` → `f` (with subscripts `f_M, f_W, f_C`) in Chapters 8–9 so no residual `ϕ` appears anywhere in the thesis once §5.6 is removed. Recommended for zero ambiguity, but not strictly required.

*Concrete edits to make in v47 (in Word, hand-edit; or use the docx-unpack-pack workflow in CLAUDE.md §8.1 for structural edits):*

- **§5.6 Variables** — delete the paragraph beginning *"A note on the phi variable."* Update any preceding sentence that counts variable types ("seven" → "six", or strip the count entirely).
- **Annex D.2 Variables by type** — delete the paragraph that contains *"The phi count of zero reflects the rollback of an interim grade-decomposition trial…"*. If the table above lists a phi row with count 0, delete that row too.
- **Cross-references** — grep the docx for any other mention of "phi" or "cross-node alias" outside Chapters 8–9 and remove. Likely candidates: §5.5 (foreign supply / aggregation discussion), §10.x (future work — Pedro may have flagged phi as a grade-decomposition handle; if so, the future-work paragraph still mentions grade decomposition without needing the phi term).
- **Chapters 8–9** — leave `ϕ` as-is, OR (optional) rename to `f` for cleanliness. Affected paragraphs in v45: 516, 517, 519, 520, 522, 526, 535, 536, 537, 540, 543, 545, 553, 624, 626, 628, 630, 634.

*No code changes needed.* The schema, resolver, views, and explorers are already phi-free. This is a pure thesis-prose edit; the framework as committed under tag `stage_1_complete` already matches the post-edit thesis description.

**Second item this session: PADD inventory partition audit — no zombies, but stale HTML and DB-vs-HANDOVER drift discovered.**

*Trigger.* Pedro noticed that `outputs/html/oil_network_partition_map.html` shows `padd1_refinery_stocks` with 8 partition children (`ref_american_bradford, ref_cpi_paulsboro, ref_ergon_newell, ref_monroe_trainer, ref_p66_bayway, ref_paulsboro_paulsboro, ref_pbf_delaware_city, ref_united_warren`), while the same map shows `district_RAP_refining_view` with only 3 of those refineries. Question: should those two be the same?

*Conceptual answer.* No, they were never meant to be the same — they would have captured different EIA decompositions of PADD 1 along orthogonal axes:
- `padd1_refinery_stocks` (intended) = PADD-1 **inventory** at refineries, all 8 refineries summing to EIA `MCRRSP1`.
- `district_RAP_refining_view` (real) = Appalachian-district **consumption (throughput)**, 3 refineries summing to the EIA district series; PADD 1 has two districts (RAP Appalachian + REC East Coast), splitting the 8 refineries 3+5.

*Empirical finding — the stub doesn't exist in this DB.* Full zombie audit across all node kinds and types: **219 nodes, every single one has ≥1 variable. Zero `*_refinery_stocks`, zero `*_tank_farms_pipelines`, zero orphans of any kind.** `padd1_refinery_stocks` is absent from `nodes`, `assets`, and `config/asset_graph.json`. The partition-map HTML showing it is **stale** — baked against a previous DB state that contained the decomposition nodes.

*Current PADD-inventory wiring (consistent across all 5 PADDs):*

| PADD | `padd{N}_view.inventory` | Aggregation children |
|---|---|---|
| 1 | TS `eia:MCRSTP11` | none |
| 2 | TS `eia:MCRSTP21` | Cushing hub + 3 operator sub-terminals + Patoka |
| 3 | TS `eia:MCRSTP31` | 3 Gulf hubs + St James + 4 SPR sites |
| 4 | TS `eia:MCRSTP41` | Guernsey hub |
| 5 | TS `eia:MCRSTP51` | none |

All 115 refinery nodes have `inventory = 0` (literal zero formula). Refinery-held stocks are folded into the PADD-level TS (`MCRSTP{N}` is all-of-PADD crude stocks); sub-PADD inventory points exist only where EIA publishes finer commercial-hub or SPR-site data. Internally consistent — nothing half-wired.

*Action on the stale HTML.* Regenerate the 5 canonical explorers to clear the ghost node from the partition map:

```powershell
..\.venv\Scripts\python.exe code\regenerate_htmls.py --force
```

This is documentation-only (no DB / schema / resolver changes).

*Open decision — does the thesis need the `MCRSTP = MCRRSP + MCRSFP (+SPR)` decomposition as a worked example?* Earlier handover passes (twenty-fourth pass notes) mention "PADD stock decomposition (`tank_farms_pipelines` + `refinery_stocks` aggregates materialising `MCRSTP{N} = MCRSFP{N} + MCRRSP{N} (+SPR)`)" and §5.4 of the thesis discussed it. If that prose is still in v45/46/47 and Pedro wants to keep the worked example, the decomposition would need to be wired into the DB explicitly — currently it isn't, in any PADD. If the thesis prose has dropped that detail, no DB work needed. Verify on resume.

*DB-vs-HANDOVER drift — flag for verification.* The DB state on this machine does not match the "stage 1 complete" numbers quoted at the top of this handover and in `claude/PROJECT_STATE.md`. Side-by-side:

| | HANDOVER / PROJECT_STATE claim | This DB has |
|---|---|---|
| nodes | 251 | **219** |
| variables | 1,870 | **1,694** |
| assignments | 975 | **1,690** |
| resolved rows | 291,564 | **202,591** |
| timeseries (active) | 90 | **121** |
| TS data rows | 68,793 | **15,053** |

Also, the schema on this machine has **no** `v_partition_tree`, `v_node_status`, `v_node_pcisob`, `v_node_balance_check`, `v_resolution_anomalies`, or `v_partition_sums` matviews — only 9 regular views (`v_aggregate_balance`, `v_aggregation_consistency`, `v_aggregation_edges`, `v_flow_edges`, `v_inventory_changes`, `v_node_production_sources`, `v_scope_authoritative_nodes`, `v_scope_collapsed_nodes`). Several views CLAUDE.md and PROJECT_STATE describe are missing.

Likely explanations: (a) this is a different snapshot than `stage_1_complete` (perhaps an in-progress rebuild that didn't run the full L4 view passes), (b) the documentation drifted ahead of an actual DB rollback, or (c) the `pg_restore` from the snapshot was partial. On resume, run `code/verify_state.py` and compare against the snapshot via `pg_restore` from `Oil Network Project/snapshots/oil_network_stage_1_complete.dump` to see which side is authoritative. Until reconciled, **do not quote the 251 / 1,870 / 291,564 numbers in the thesis** without checking they match the DB you're actually defending against.

*Forensic confirmation — the HTMLs on disk are from a DIFFERENT machine's DB state.*

The balance UI metadata beacon (`outputs/html/oil_network_balance_resolver.html`) declares `run_id=13, generated_at 2026-05-18T13:00:27Z`. The local `scenario_resolver_runs` table only has runs 1, 2, 3 — all from **2026-05-12**. Dispatch stats on the local runs (76 observed + 408 alias + 738 latent + 5 closure + 8 arithmetic + 449 zero + 1 sum_same_type + 4 sum_over_children + 1 sum_over_outflows + 0 unresolved) do not match HANDOVER's claimed stats. Net interpretation: the OneDrive-synced HTML files reflect another machine's stage_1_complete DB state; the local DB is an earlier configuration that never advanced to stage_1_complete. Two consequences:

- The screenshots Pedro can navigate via `file://...` show the canonical stage-1 state (with `padd{N}_refinery_stocks` / `padd{N}_tank_farms_pipelines` nodes and the full L4 view set).
- The local DB has a *simpler* design: `padd{N}_view.inventory` is TS-bound directly to `MCRSTP{N}` with no intermediate stock-decomposition nodes; refinery inventory is uniformly 0; hub/SPR-site inventories partition into `padd{N}_view.inventory` directly (no intermediate `padd{N}_refinery_stocks` layer).

**Third item this session: design clarification triggered by the screenshot — KEEP the `padd{N}_refinery_stocks` / `padd{N}_tank_farms_pipelines` decomposition, but reframe the thesis prose as a multi-series-composition worked example. Earlier in the session I argued for Path A (drop the decomposition); Pedro reversed it on reflection. The reversal is the right call. The Path A reasoning is preserved below as superseded context, then the corrected direction.**

*Trigger.* Pedro's screenshot of `oil_network_balance_resolver.html` at PADD 4 / 2025-04-01 showed `padd4_refinery_stocks` (S=2,349 mbbl, ΔS=-4.9) with 9 PADD-4 refineries as partition children, and `padd4_tank_farms_pipelines` (S=22,452 mbbl, ΔS=+22.6) as a parallel sub-aggregate, plus the **same 9 refineries appearing again as direct children of padd4_view**. Pedro's question: *"if I have a padd_view on refineries, then I should have the refineries and see the refinery stocks under that — why do I need a view to see stocks, another one for production, etc?"*

*Answer — Pedro is right.* The intermediate nodes contradict Principle 2.4 (formula-implies-relation; variables collection is the single source of truth). Each `padd{N}_view` variable already has its own partition formula; the partition children are the asset nodes that contribute to that specific variable. There is no need for an intermediate "view node" that groups children by variable type — variable type already *is* the grouping mechanism. Creating a `padd4_refinery_stocks` node whose sole purpose is to bundle "refineries that contribute inventory to PADD 4" promotes a property-of-variables (which assets carry an inventory variable) into a structural object in the graph. That's a category-as-node mistake.

*Where it came from.* The decomposition matches an EIA series convention: `MCRSTP{N} = MCRRSP{N} + MCRSFP{N} (+SPR)`. Stage-1 versions of the model wired this as structure so the asset graph would line up with EIA's published aggregates. The narrative value is real; the structural value is zero — the partition could go directly from `padd{N}_view.inventory` to individual refinery/hub/SPR inventories without losing any information.

*Visible cost in the current HTML.* Each PADD-4 refinery appears twice in the partition tree — once under `padd4_refinery_stocks` (inventory contribution), once under `padd4_view` directly (consumption contribution via the refining-district views or direct partition). The UI shows both, which breaks the clean-tree intuition readers expect from a partition map.

*Recommended fix — Path A: drop the decomposition layer.* This matches the local DB's simpler design, which the framework can defend just as well. Concretely:

1. **Schema / DB.** Remove `padd{N}_refinery_stocks` and `padd{N}_tank_farms_pipelines` nodes from the canonical `config/asset_graph.json` (they're already absent from this machine's DB). Have `padd{N}_view.inventory` TS-bound to `MCRSTP{N}` directly, with optional partition formula spanning hub/SPR-site inventories where finer data exists (current local design already does this — Cushing+sub-hubs+Patoka for PADD 2; Gulf hubs + St James + 4 SPR sites for PADD 3; Guernsey for PADD 4; nothing finer for PADDs 1 and 5).
2. **Thesis prose.** Drop / rewrite the §5.4 paragraph that motivates the structural decomposition. Keep the EIA series alignment as a *narrative* point (one paragraph: "EIA publishes the PADD-level total `MCRSTP{N}` and the sub-aggregates `MCRRSP{N}` for refinery-held stocks and `MCRSFP{N}` for tank-farm + pipeline stocks; the framework binds the PADD total directly because the sub-aggregates contain no information not already present at the named-asset level"). This frames the decomposition as a known-but-unused EIA convention, not a load-bearing structural choice.
3. **HTMLs.** Regenerate from the local DB once the prose decision is settled, so the partition map and balance UI match the simplified structure: `..\.venv\Scripts\python.exe code\regenerate_htmls.py --force`.
4. **Reference PDFs.** `Resolver_Walkthrough.pdf`, `Scenario_Construction.pdf`, `Graph_Construction.pdf`, `Design_Principles.pdf` — check the markdown source for any mention of the decomposition nodes; rebuild if found.

*Alternative — Path B (NOT recommended unless the EIA-alignment story is load-bearing for the defence).* Keep the decomposition nodes, but make them disciplined: each node carries inventory only (P=C=F_in=F_out=B=0 explicitly); refineries appear under `*_refinery_stocks` only and reach `padd{N}_view` via that single path for inventory; consumption goes through `district_X_refining_view` only; no double-parenting in the partition tree. This is more work and harder to defend as a clean asset-centric model.

*On resume, the open question is which design Pedro wants to defend.* If Path A (recommended): thesis-prose edit in §5.4 plus HTML regeneration. No code work — the local DB already matches. If Path B: schema work to add the decomposition nodes back cleanly, plus DB rebuild from the snapshot to recover the named refineries that the stage_1_complete DB has but the local DB lacks. Path A is consistent with the simplification Pedro has been driving in v45–v47 (phi removal, single-variable-type partitions, drop of cross-node aliasing).

*Pedro's reversal — final direction.* Pedro pushed back on Path A with the framing: *"keep the current decomposition at the model to show that we can combine time series"*. That reframe is correct and the right call. Reasoning:

- EIA publishes **three related series** for PADD-level crude stocks: `MCRSTP{N}` (PADD total), `MCRSFP{N}` (tank farms + pipelines), `MCRRSP{N}` (refineries), satisfying the identity `MCRSTP = MCRSFP + MCRRSP (+SPR for PADD 3)`.
- The framework ingests **all three** as TS bindings on three different nodes (`padd{N}_view`, `padd{N}_tank_farms_pipelines`, `padd{N}_refinery_stocks`) and cross-validates the identity via `v_aggregation_consistency`. Each sub-aggregate has its own constituent list (`formula_inputs`) of named hubs / SPR sites / refineries that contribute to it.
- The decomposition also enables the **Jones Act in-transit derivation** (`refactor_jones_act_into_inter_padd_agg.py`): `inter_padd_3_to_5_agg.inventory = padd5_view.inventory − padd5_tank_farms_pipelines.inventory − padd5_refinery_stocks.inventory`, capturing crude on tankers between Gulf and West Coast as a residual. This is a real, operationally meaningful signal worth hundreds of mbbl.
- This is a **textbook example of Principle 2.8's "formula_inputs as constraint set when TS-bound"**: three TS-observed values, framework verifies they satisfy the EIA identity, residual emerges naturally. Not a category-as-node mistake — each decomposition node corresponds to an externally-published EIA series.

*Path A was wrong because it conflated two patterns:* (a) inventing a node whose only purpose is to group children that happen to carry one variable (a real mistake — would violate Principle 2.4); (b) modelling an externally-published sub-aggregate that has its own TS and its own constituent partition (the actual situation here — perfectly aligned with Principle 2.8). Pattern (b) is a feature of the framework, not redundancy.

*UI artefact acknowledgment.* The partition map shows refineries under two parents because each refinery contributes to two different variables of two different parents: (zero) *inventory* to `padd{N}_refinery_stocks`, *consumption* to `padd{N}_view` via the refining-district views. Two variables, two partitions, one fact each — Principle 2.4 is satisfied. The UI just renders the union. Worth a sentence in the thesis acknowledging this, not a structural fix.

*Concrete edits in v47/v48 thesis prose (replacing the Path A edits proposed earlier):*

- **§5.4** — KEEP the PADD stock-decomposition section. Reframe its motivation: instead of "structural decomposition", lead with *"EIA publishes the PADD-level total `MCRSTP{N}` alongside two sub-aggregates `MCRSFP{N}` and `MCRRSP{N}` satisfying `MCRSTP = MCRSFP + MCRRSP (+SPR)`. The framework ingests all three as separate TS bindings on three nodes, allowing the identity to be cross-validated via `v_aggregation_consistency`. The Jones Act in-transit residual emerges as `padd5_view − padd5_tank_farms_pipelines − padd5_refinery_stocks`."*
- **§4 (axioms / corollaries chapter)** — consider adding a one-paragraph generalisation: *"any time multiple TS observe the same physical quantity at different granularities, the framework can hold all of them on a tree of nodes with the parent TS-bound to the coarsest series and each child TS-bound to its own series; `formula_inputs` then acts as the constraint set and the identity is cross-validated."* Frame this as a concrete realisation of Principle 2.8.
- **partition-map UI note** — one sentence in §5 or §6: *"physical assets may appear under multiple aggregate parents in the partition map; each instance corresponds to a different variable contribution (a refinery contributes consumption to `padd{N}_view` and inventory to `padd{N}_refinery_stocks`)."*

*DB reconciliation needed.* The local DB on this machine (Stage1) is **BEHIND stage_1_complete** — it never ran the decomposition migration pass (or several later passes). Numbers: 219 nodes vs 251 (canonical), 1,694 vars vs 1,870, 1,690 assignments vs 975, 202,591 resolved rows vs 291,564, latest run_id=3 from 2026-05-12 vs run_id=13 from 2026-05-18. The schema also lacks `v_partition_tree`, `v_node_status`, and four other L4 views. The on-disk HTMLs at `outputs/html/` were generated against the canonical state and are correct; the local DB cannot regenerate them as-is.

*Restoration recipe (on resume, before any further work):*

```powershell
$env:PGPASSWORD = 'eia_password'
& 'C:\Program Files\PostgreSQL\18\bin\pg_restore.exe' -h localhost -U eia_user -d eia_crude `
  -n oil_network --clean --if-exists `
  '..\snapshots\oil_network_stage_1_complete.dump'
..\.venv\Scripts\python.exe code\verify_state.py
```

Expected after restore: 251 nodes, 1,870 variables, 291,564 resolved rows, latest run_id=13, plus the full L4 view set. After verification, `regenerate_htmls.py --list` should report all 5 HTMLs as "current" (run_id=13 = latest). No regeneration needed.

*Backup of pre-restoration HTMLs preserved at* `outputs/html/_backup_runid13_20260521/` *— this is the same content as the live HTMLs (since the failed regeneration attempt didn't overwrite them except for `map_resolver.html`, which I restored from the backup). Safe to delete after restore confirms HTMLs are consistent with the canonical DB.*

**Fourth item this session: full rebuild + verification + thesis archive done. End-to-end consistency confirmed.**

*Rebuild path taken — snapshot restore (not orchestrator rerun).*

1. `DROP SCHEMA oil_network CASCADE; CREATE SCHEMA oil_network;` (the local schema had regular views where the snapshot expects matviews — incompatible without a full drop).
2. `pg_restore -h localhost -U eia_user -d eia_crude -n oil_network ..\snapshots\oil_network_stage_1_complete.dump` (no flags beyond `-n` since the schema was empty).
3. `code\verify_state.py` — passes cleanly.
4. `code\regenerate_htmls.py --force` — all 5 HTMLs regenerated cleanly (the run_id=13 beacon on the on-disk HTMLs matched the latest resolver run, so the orchestrator initially reported them as "current"; `--force` rebuilt them anyway against the freshly-restored DB).
5. Both `oil_network_metadata` and `oil_network_data_loader` schemas survived the drop (they sit alongside, not inside, `oil_network`). The metadata schema's `metadata_production_assets` table had a FK to `oil_network.assets` that was dropped by CASCADE; if any downstream queries fail on resume, that FK is the suspect — re-create it after the restore or run the metadata-loader migration that originally added it.

*Headline numbers post-restore (all match HANDOVER claims / PROJECT_STATE):*

| | Claimed / expected | Actual |
|---|---|---|
| nodes / assets | 251 | **251** ✓ |
| variables | 1,870 | **1,870** ✓ |
| variable_assignments | 975 | **975** ✓ |
| scenario_resolved_values | 291,564 | **291,564** ✓ |
| timeseries (catalogue) | 137 (90 active) | **137** ✓ |
| timeseries_data | 68,793 | **68,793** ✓ |
| resolver runs in table | up to run_id=13 | **13** ✓ |
| latest run dispatch | 90 obs + 542 zero + 767 latent + 451 arith + 5 sum + 15 rev_mirror | **identical** ✓ |
| unresolved rows | 0 | **0** ✓ |
| TS-binding collisions | 0 | **0** ✓ |
| capacity violations | 0 | **0** ✓ |
| matviews | 12 | **12** ✓ (`v_aggregate_balance, v_aggregation_consistency, v_aggregation_edges, v_flow_edges, v_formula_input_links, v_inventory_changes, v_node_balance_check, v_node_pcisob, v_node_status, v_partition_intra_flows, v_partition_sums, v_partition_tree`) |

*HTMLs are now internally consistent AND match the canonical DB:*

| view | size | generated_at | embedded run_id |
|---|---|---|---|
| partition_map | 98 KB | 2026-05-21T19:49:51Z | 13 |
| node_neighbors_map | 326 KB | 2026-05-21T19:49:52Z | 13 |
| balance_resolver | 1494 KB | 2026-05-21T19:49:53Z | 13 |
| hierarchy_resolver | 724 KB | 2026-05-21T19:49:54Z | 13 |
| map_resolver | 141 KB | 2026-05-21T19:49:54Z | 13 |

*Multi-series-composition (the decomposition rationale) confirmed in the data — both PADDs:*

PADD 4 @ 2025-04-01 (EIA identity holds exactly):
- `padd4_view.inventory` = **24,801 mbbl** (TS MCRSTP4)
- `padd4_refinery_stocks.inventory` = **2,349 mbbl** (TS MCRRSP4) — matches screenshot exactly
- `padd4_tank_farms_pipelines.inventory` = **22,452 mbbl** (TS MCRSFP4) — matches screenshot exactly
- Sum of sub-aggregates = 24,801. **Residual = 0.0.**

PADD 5 @ 2025-04-01 (Jones Act in-transit residual is real and substantial):
- `padd5_view.inventory` = **48,608 mbbl** (TS MCRSTP5)
- `padd5_refinery_stocks.inventory` = **21,220 mbbl**
- `padd5_tank_farms_pipelines.inventory` = **24,491 mbbl**
- Residual = 48,608 − 21,220 − 24,491 = **2,897 mbbl** of crude in-transit between Gulf and West Coast on Jones Act tankers.
- `inter_padd_3_to_5_agg.inventory` = **2,897 mbbl** (matches residual exactly).

These two cases together are the worked example to keep in the thesis §5.4 prose: PADD 4 demonstrates the EIA identity holding exactly when there is no in-transit term, and PADD 5 demonstrates the framework deriving an operationally-meaningful in-transit signal as a residual.

*Thesis-version archive done.*

Moved 45 thesis files older than 3 days (cutoff 2026-05-18 20:52) from `outputs/docs/` to `outputs/docs/old/`. Total archived: 39.3 MB. `outputs/docs/` now contains only 9 thesis files within the last 3 days:

```
v37 PP.docx     (2026-05-19)
v37 PP1.docx    (2026-05-19)
v38.docx        (2026-05-19)
v39.docx        (2026-05-19)
v40.docx        (2026-05-19)
v44.docx        (2026-05-19)
v45.pdf         (2026-05-19)
v45.docx        (2026-05-21)
v46.docx        (2026-05-21, latest)
```

Reference PDFs (`Design_Principles.pdf`, `Resolver_Walkthrough.pdf`, `Scenario_Construction.pdf`, `Graph_Construction.pdf`), figures directory, References directory, and Annex docs are untouched — only the dated `Master_Thesis_Pedro_Porfirio_v*.{docx,pdf}` series was archived.

*To pick up on the other machine on resume.* `git pull origin main`. The DB on the other machine should already be at stage_1_complete (it's the one that generated the HTMLs in the first place). Run `code\verify_state.py` to confirm; if numbers don't match the table above, restore from `..\snapshots\oil_network_stage_1_complete.dump` (after dropping `oil_network` schema if it has incompatible relations). Then continue with the thesis prose edits — phi removal in §5.6 + Annex D.2, §5.4 reframing as multi-series composition, optional `ϕ → f` rename in Chapters 8–9.

*Couldn't read v46 from disk during the session* — python-docx failed with `PackageNotFoundError` despite the file existing at 1.3 MB. Most likely cause: OneDrive sync state or Word having a transient lock from the v47 save-in-progress. v45 read cleanly; the phi text in v45 §5.6 / D.2 is the canonical reference for the edits above. On resume, first verify the phi paragraphs are still present in v46/v47 (Word may have already been edited locally).

**To pick up on the other machine.**

1. `git pull origin main` (this handover update will be the latest commit if Pedro commits it before switching).
2. Open the latest thesis docx in Word (v47 if saved by then, else v46).
3. Apply the §5.6 and Annex D.2 deletions above; optionally rename Chapter 8–9 `ϕ` → `f`.
4. Save as v48 (or whichever next version), generate paired PDF for the Remarkable per the standard workflow.
5. No DB rebuild, no resolver rerun, no HTML regeneration needed — this is documentation-only.

---

## Earlier — superseded resume entry (2026-05-18 evening — stage 2 attempt rolled back; airport-pickup state)
